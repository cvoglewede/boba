...
def statcast_h_season(season):
    ...
    data = df_all[['year', 'player_id', 'player_name', 'pitch_count',
                'pa',
                'qualify_pa',
                'qualify_ip',
                'in_zone_percent',
...
def gather_season_data(position_group,source, start_season, end_season):
    base_path = 'data/raw/'+position_group+'/'+source+'/season/'
    files = []
    seasons_list = list(np.arange(start_season,end_season,1))
    data = pd.DataFrame() 
    for x in seasons_list:
        path = base_path+str(x)+'.csv'
        df = pd.read_csv(path,index_col=0)
        headings = df.columns
        data = data.append(df)
    data.columns = headings
    aggregated_path = base_path +'master.csv'
    data.to_csv(aggregated_path, index = False)
    return data

def gather_proj_sys_data(position_group,start_season, end_season):
    base_path = 'data/raw/'+position_group+'/'+source+'/season/'
    files = []
    seasons_list = list(np.arange(start_season,end_season,1))
    data = pd.DataFrame() 
